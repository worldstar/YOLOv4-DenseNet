...
lossname        = 'yolo_loss'
if modeltype == "YOLOV4" or modeltype == "CSPYOLOV4Densenet":
    lossname = 'yolov4_loss'
...
def _main():
    ...
    if True:
        ...
        model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
            ...
            validation_data=data_generator_wrapper(val_lines[:num_val], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
        ...
        # model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
        #     steps_per_epoch=max(1, num_train//batch_size),
        #     validation_data=data_generator_wrapper(lines[num_train:], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
        #     validation_steps=max(1, num_val//batch_size),
        #     epochs=epoch,
        #     initial_epoch=0,
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    if modeltype == "YOLOV3-SPP":
        model_body = yolo_body(image_input, num_anchors//3, num_classes,SPP=True)
    if modeltype == "YOLOV3Densenet":
        model_body = densenet_body(image_input, num_anchors//3, num_classes)
    if modeltype == "CSPYOLOV3Densenet":
        model_body = densenet_body(image_input, num_anchors//3, num_classes,CSP = True)
    if modeltype == "YOLOV3SPPDensenet":
        model_body = densenet_body(image_input, num_anchors//3, num_classes,SPP=True)
    ...
    if modeltype == "CSPYOLOV4Densenet":
        model_body = yoloV4densenet_body(image_input, num_anchors//3, num_classes,CSP = True)
    if modeltype == "CSPSPPYOLOV3Densenet":
        model_body = densenet_body(image_input, num_anchors//3, num_classes,CSP = True,SPP = True)
    print('Create {} model with {} anchors and {} classes.'.format(modeltype,num_anchors, num_classes))
    ...
    if load_pretrained:
        ...
        if modeltype == "YOLOV3Densenet":
            if freeze_body in [1, 2]:
                # Do not freeze 3 output layers.
                #num = len(model_body.layers)-7
                num = (185, len(model_body.layers))[freeze_body-1]
                for i in range(num): model_body.layers[i].trainable = False
                print('Freeze the first {} layers of total {} layers.'.format(num, len(model_body.layers)))
        if modeltype == "YOLOV3SPPDensenet":
            if freeze_body in [1, 2]:
    ...
    if modeltype == "YOLOV4" or modeltype == "CSPYOLOV4Densenet":
        model_loss = Lambda(yolov4_loss, output_shape=(1,), name=lossname,
        arguments={'anchors': anchors, 'num_classes': num_classes, 'ignore_thresh': 0.5})(
        [*model_body.output, *y_true])
    else:
        model_loss = Lambda(yolo_loss, output_shape=(1,), name=lossname,
        arguments={'anchors': anchors, 'num_classes': num_classes, 'ignore_thresh': 0.5})(
        [*model_body.output, *y_true])
        ...
        for b in range(batch_size):
            ...
            if mosaic:
                if flag and (i+4) < n:
                    ...
                    image, box = get_random_data_with_For_Mosaic(annotation_lines[i], input_shape)
                    i = (i+1) % n
                flag = bool(1-flag)
            else:
